"""Module providing a collection of functions to generate openhab things/items/sitemap from a knxproject"""
import re
import os
import logging
import shutil
...
def _load_template(template_name):
    ...
    1. Package resources via importlib (pip install)
    2. Direct file path in src/knx_to_openhab/templates/ (development)
    3. Fallback to root directory (backwards compatibility)
    
    Args:
        template_name (str): Name of template (e.g. 'things', 'items', 'sitemap')
    
    Returns:
        str: Template content
    
    Raises:
        FileNotFoundError: If template cannot be found using any strategy
    
    Example:
        things_template = _load_template('things')
        items_template = _load_template('items')
        sitemap_template = _load_template('sitemap')
    """
    # Strategy 1: importlib.resources (pip install / installed packages)
    try:
        template_dir = files('knx_to_openhab').parent / 'templates'
        template_path = template_dir / f'{template_name}.template'
        return template_path.read_text('utf8')
    except Exception as e:
        logger.debug(f"Strategy 1 (importlib.resources) failed for '{template_name}': {e}")
    
    # Strategy 2: Direct file in src/knx_to_openhab/templates/ (development environment)
    try:
        ...
        if os.path.isfile(src_path):
            with open(src_path, 'r', encoding='utf8') as f:
                logger.debug(f"Loaded template '{template_name}' from: {src_path}")
                return f.read()
    except Exception as e:
        logger.debug(f"Strategy 2 (direct file) failed for '{template_name}': {e}")
    
    # Strategy 3: Fallback to root (backwards compatibility with old structure)
    try:
        ...
        if os.path.isfile(root_path):
            with open(root_path, 'r', encoding='utf8') as f:
                logger.debug(f"Loaded template '{template_name}' from root: {root_path}")
                return f.read()
    except Exception as e:
        logger.debug(f"Strategy 3 (root fallback) failed for '{template_name}': {e}")
    
    # All strategies failed
    raise FileNotFoundError(
        f"Template '{template_name}.template' not found. "
        f"Searched: importlib.resources, src/knx_to_openhab/templates/, project root. "
        f"Ensure template file exists in one of these locations."
    )

...
@pytest.mark.parametrize("project_file", PROJECT_FILES[:1])  # Only test with first file to avoid excessive runs
def test_full_generation_with_mocked_dependencies(project_file, mock_config, tmp_path, caplog):
    ...
    with patch('argparse.ArgumentParser.parse_args', return_value=test_args), \
        ...
        errors = [r for r in caplog.records if r.levelname in ('ERROR', 'CRITICAL')]
        assert len(errors) == 0, f"No errors should occur during generation: {[r.message for r in errors]}"
...
def test_full_generation_output_content_validation(project_file, mock_config, tmp_path, caplog):
    ...
    with patch('argparse.ArgumentParser.parse_args', return_value=test_args), \
        ...
        assert 'Group' in items_content or 'Switch' in items_content or 'Dimmer' in items_content, \
        ...
        assert 'Thing' in things_content or 'Bridge' in things_content or 'knx:' in things_content.lower(), \
        ...
        assert 'sitemap' in sitemap_content.lower() or 'Frame' in sitemap_content or 'Text' in sitemap_content, \
